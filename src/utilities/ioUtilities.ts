...
function reversedFaceToLines(face: RubikFaceModel): string[] {
  const rows = [...face.cells].reverse();
...
function reversedBlocksToFace(
  ...
  const reversedRows = [...rows]
    .reverse()
    .map((row) => [...row].reverse()) as RubikFaceModel['cells'];

  return { name, cells: reversedRows };
}

export function toFlattenStyleString(rubik: RubikModel): string {
  const up = reversedFaceToLines(rubik.U);
...
export function fromFlattenStyleString(value: string): RubikModel {
  ...
  const up = reversedBlocksToFace('U', [
    blocksByLine[0][2],
    blocksByLine[1][2],
    blocksByLine[2][2],
  ]);
